...
public class HttpMessageHandler {
    ...
    private String prepararMensagem(Exception ex, List<JsonMappingException.Reference> path) {
        ...
        for (int i = 0; i < path.size(); i++) {
            ...
            if (Objects.nonNull(campo)) {
                ...
                grupos =
                        Objects.isNull(grupos)
                                ? campo
                                : grupos.concat(".").concat(path.get(i).getFieldName());
            }
        }
        ...
        mensagem =
                format(
                        MensagemUtils.getMensagem(("Lista.generico.invalido")),
                        propriedade,
                        grupos,
                        valorErro);
        ...
        if (Objects.isNull(grupos)) {
            mensagem =
                    format(
                            MensagemUtils.getMensagem(("Campo.generico.invalido")),
                            propriedade,
                            valorErro);
        }
        return mensagem;
    ...
    private String obterValorErro(Exception ex) {
        try {
            JsonParser parser =
                    (JsonParser) ((MismatchedInputException) ex.getCause()).getProcessor();
